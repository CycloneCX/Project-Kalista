...
namespace Kalista
{
    public class ActiveModes
    {
        ...
        public static void OnPermaActive()
        {
            ...
            {
                var rendTarget =
                    HeroManager.Enemies.Where(x => x.IsValidTarget(E.Range) && E.GetDamage(x) > 1 && !x.HasNoDmgBuff())
                ...
                if (Config.BoolLinks["miscKillstealE"].Value &&
                    rendTarget != null
                            && Damages.GetActualDamage(rendTarget) >= Damages.GetActualHealth(rendTarget)
                            && !rendTarget.IsDead && Environment.TickCount - E.LastCastAttemptT > 500
                            && !rendTarget.HasNoDmgBuff())
                {
                    E.Cast();
                ...
                else if (Config.BoolLinks["miscAutoEchamp"].Value)
                {
                    ...
                    if (enemy != null)
                    {
                        ...
                        {
                            if (ObjectManager.Get<Obj_AI_Minion>().Any(o => E.IsInRange(o) && E.CanCast(o) && o.Health <= E.GetDamage(o)))
                            {
                                E.Cast();
                                E.LastCastAttemptT = Environment.TickCount;
                            }
			...
			
            {
                ...
                var rendTarget =
                    HeroManager.Enemies.Where(
                        x =>
                        x.IsValidTarget(E.Range) && E.GetDamage(x) > 1 && !x.HasNoDmgBuff())
                        .OrderByDescending(x => E.GetDamage(x))
                        .FirstOrDefault();
                ...
                if (E.IsEnabled(Mode.COMBO) && (E.Instance.State == SpellState.Ready || E.Instance.State == SpellState.Surpressed) && target.HasRendBuff())
                {
                    ...
                    if (player.Distance(target, true) > Math.Pow(Orbwalking.GetRealAutoAttackRange(target), 2))
                    {
                        ...
                        if (minions.Any(m => E.CanCast(m) && m.Health <= E.GetDamage(m)))
                        {
                            E.Cast();
                            E.LastCastAttemptT = Environment.TickCount;
                        }
                        else
                        {
                            ...
                            var minion = VectorHelper.GetDashObjects(minions).Find(m => m.Health > player.GetAutoAttackDamage(m) && m.Health < player.GetAutoAttackDamage(m) + Damages.GetRendDamage(m));
                            if (minion != null)
                            {
        ...
        public static void OnWaveClear()
        {
            ...
            {
                var minionkillcount =
                    MinionManager.GetMinions(E.Range).Count(x => E.CanCast(x) && x.Health <= E.GetDamage(x));

                if ((minionkillcount >= Config.SliderLinks["waveNumE"].Value.Value))
                {
